...
use shared::{color::RGBA, map::Cell, GameState};
...
pub fn fill_disconnected(data: &mut [u8]) {
    const DISCONNECTED_COLOR: RGBA = (220, 20, 60, 255);
    fill_background(data, TORUS_INTERVAL, DISCONNECTED_COLOR);
}

fn fill_texture(level: usize, data: &mut [u8], game_state: &Option<GameState>, blackish: RGBA) {
    match &game_state {
        None => {
            if level == 0 {
                fill_disconnected(data);
            }
        }
        Some(game_state) => {
            ...
            for map_y in 0..h {
                ...
                for map_x in 0..w {
                    ...
                    if level == 0 {
                        ...
                        fill_cell(level, data, cell, cell_range);
                    }
                }
            }
        }
...
pub fn update_texture(
    ...
    query: Query<(&Handle<StandardMaterial>, &Torus)>,
